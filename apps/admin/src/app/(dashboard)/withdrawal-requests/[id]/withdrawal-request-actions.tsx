...
export function WithdrawalRequestActions({
  id,
  status,
  isSelf,
  displayName,
  ...
  async function reject(comment: string) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/proxy/admin/withdrawal-requests/${id}/reject`, {
        method: "POST",
  ...
  async function approve() {
    ...
    setError("");
    try {
      const res = await fetch(`/api/proxy/admin/withdrawal-requests/${id}/approve`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });
      const data = await res.json();
    ...
    <div className="flex flex-col items-end gap-1">
      <div className="flex items-center gap-2">
        ...
        <RejectDialog
          ...
          description={
            isSelf
              ? `ATTENTION: c'est VOTRE demande. ${displayName ?? email} (${formatHtg(amountCents)}, ${method})`
              : `${displayName ?? email} (${formatHtg(amountCents)}, ${method})`
          }
          onConfirm={reject}
          trigger={
