...
const STATUSES = ["pending", "processing", "successful", "failed", "cancelled", "all"];

type WithdrawalRequestsFilterProps = {
  status: string;
  page: number;
  total: number;
  perPage: number;
...
export function WithdrawalRequestsFilter({ status, page, total, perPage }: WithdrawalRequestsFilterProps) {
  ...
  function push(updates: { status?: string; page?: number }) {
    ...
    router.push(qs ? `/withdrawal-requests?${qs}` : "/withdrawal-requests");
  }
